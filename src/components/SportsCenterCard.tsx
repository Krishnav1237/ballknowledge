...
/**
 * Maps raw verdict strings generated by the AI grader to premium, human-readable tribunal labels.
 * Considers both the text pattern of the verdict and the OVR rating to categorise users.
 * 
 * @param {string} verdict - The raw verdict string (e.g. "CERTIFIED COOKING").
 * @param {number} ovr - Overall rating score (1-99).
 * @returns {string} Cleaned up label for the collectible card display.
 */
function getVerdictLabel(verdict: string, ovr: number): string {
  ...
  // OVR fallback if no keyword matches
...
// ─── Verdict Color ───────────────────────────────────────────────────────────
/**
 * Resolves color codes and radial drop shadow glow values matching the mapped verdict stamp.
 * Green stamps represent high tier performance, orange/red represent bad/delusional takes.
 * 
 * @returns {{color: string, glow: string}} Hex color code and CSS rgba glow value.
 */
...
/**
 * Returns the theme visual colors, gradients, and filters.
 * Hardcoded to gold/burgundy premium aesthetic.
 */
...
/**
 * extracts individual PRD, MGR, HOT, and RST ratings from verdict data.
 * Sequential fallback chain:
 * 1. `statsJson` object (V1 format)
 * 2. `stats` array (V0 format)
 * 3. Fallback fields from local predictions
 * 4. Overall OVR rating as a direct fallback
 * 
 * @param {VerdictData} data - The raw card/verdict data payload.
 * @returns {Array<{label: string, val: number}>} Array containing exactly 4 metrics.
 */
...
/**
 * Maps standard World Cup countries flag emojis to matching jersey styles.
 * Returns primary, secondary, collar hex colors, and pattern type (stripes, solids, halves, checkers).
 * 
 * @param {string} flag - The country flag emoji (e.g. "🇦🇷").
 * @returns {Object} Primary, secondary, collar colors, and styling rules.
 */
function getJerseyDetails(flag: string) {
  ...
  const colorFallbacks = [
    { primary: '#1E293B', secondary: '#E11D48', collar: '#E11D48', style: 'solid' }, // black-red
  ...
  return (
    <svg className="w-[180px] h-[180px] drop-shadow-[0_8px_16px_rgba(0,0,0,0.65)]" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        {/* Jersey 3D shading gradient */}
        <linearGradient id={`jersey-shading-${cleanId}`} x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <g mask={`url(#${maskId})`}>
        ...
        {/* Shading/Lighting Gradient overlay on Jersey */}
        <path 
          d="M 38 20 Q 50 28 62 20 L 78 24 L 92 42 L 82 52 L 74 42 L 74 92 L 26 92 L 26 42 L 18 52 L 8 42 L 22 24 Z" 
          fill={`url(#jersey-shading-${cleanId})`} 
      ...
      {/* Shadow under collar for 3D neck depth */}
      <path 
        d="M 37.5 19.5 Q 50 28.5 62.5 19.5 Q 50 32 37.5 19.5" 
        fill="#000000" 
        opacity="0.35" 
      />
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Soft highlight glow behind the player cutout area */}
        <ellipse cx="162" cy="160" rx="55" ry="70" fill="url(#gold-glow)" opacity="0.45" style={{ mixBlendMode: 'screen' }} />
      ...
      <div className="absolute inset-0 w-full h-full text-white pointer-events-none">
        ...
        <div className="absolute top-[25px] left-[22px] w-[55px] flex flex-col items-center pointer-events-none">
          ...
          {/* Ball Knowledge Logo Badge */}
          <div className="flex flex-col items-center mt-2">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/images/ball_knowledge_logo.png"
              alt="Ball Knowledge"
              className="w-[34px] h-[34px] object-contain drop-shadow-[0_1.5px_4px_rgba(0,0,0,0.9)] rounded-full"
            />
          </div>
        </div>
        ...
        >
          {hasCustomPhoto ? (
            ...
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={`https://api.dicebear.com/7.x/${(data.avatarStyle && !data.avatarStyle.startsWith('ai-')) ? data.avatarStyle : 'fun-emoji'}/svg?seed=${encodeURIComponent(data.avatarSeed || data.playerName || 'TAKE MAKER')}&backgroundColor=transparent`}
              alt="Avatar"
          ...
          {metrics.map((m) => {
            ...
            return (
              <div key={m.label} className="flex flex-col items-center flex-1">
                <span 
                  className="text-[9.5px] font-black tracking-widest uppercase leading-none flex flex-col items-center gap-0.5" 
                  style={{ color: colors.textColor || '#FBBF24' }}
                >
                  <span>{m.label}</span>
                  <span className="text-[7px] opacity-70 font-normal">{getWeightLabel(m.label)}</span>
                </span>
                <span 
                  className="text-[19px] mt-1 leading-none font-black text-white drop-shadow-[0_1.5px_3px_rgba(0,0,0,0.85)]"
                  style={{ 
                    fontFamily: "'Oswald', sans-serif",
                    fontWeight: 900
                  }}
