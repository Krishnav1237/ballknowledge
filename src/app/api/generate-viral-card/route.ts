...
function buildCompleteFifacardPrompt(params: {
  ...
  return (
    ...
    `Framing & Shot Type: The camera must be zoomed out further to a classic upper-torso portrait. The player's head, full neck, shoulders, and chest must be fully visible. There must be a large amount of empty headroom space (at least 20% of the vertical canvas height) above the top of their head so that their face is positioned lower down, in the middle-upper center of the canvas, and does not touch or overlap with any elements near the top edge. The player's head must occupy only about 18% to 20% of the vertical canvas height, showing a large portion of their national jersey below it. ` +
